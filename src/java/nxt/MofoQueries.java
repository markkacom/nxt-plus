            ...
            PreparedStatement pstmt = con.prepareStatement(b.toString());)
        {
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    list.add(json);
                    json.put("timestamp", String.valueOf(rs.getInt("r")));
                    json.put("avg", String.valueOf(rs.getLong("avg")));
                    json.put("low", String.valueOf(rs.getLong("low")));
                    json.put("high", String.valueOf(rs.getLong("high")));
                    json.put("vol", String.valueOf(rs.getLong("vol")));
                    
                    String price_concat = rs.getString("price_concat");
                    String[] prices = price_concat.split(",");
                    if (prices.length > 0) {
                        json.put("open", prices[0]);
                        json.put("close", prices[prices.length-1]);
                    }*/
